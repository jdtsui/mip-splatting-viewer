...
int main(int ac, char** av) 
{
	...
	rng = findArg(cfgLine, "kernel_size");
	float kernel_size = std::stof(cfgLine.substr(rng.first, rng.second - rng.first));
	...
	rendering_width = 4000;
	rendering_height = 2260;
	...
	// Create the ULR view.
	GaussianView::Ptr	gaussianView(new GaussianView(scene, sceneResWidth, sceneResHeight, plyfile.c_str(), &messageRead, sh_degree, white_background, kernel_size, !myArgs.noInterop, device));
	...
	// Add views to mvm.
	MultiViewManager        multiViewManager(window, true);

	if (myArgs.rendering_mode == 1) 
		multiViewManager.renderingMode(IRenderingMode::Ptr(new StereoAnaglyphRdrMode()));
	
	multiViewManager.addIBRSubView("Point view", gaussianView, usedResolution, ImGuiWindowFlags_NoBringToFrontOnFocus);
