	...
	sibr::Vector3f& maxx)
{
	...
	std::vector<float> filter_3Ds;
	std::vector<Scale> temp_scales;
	// Resize our SoA data
	pos.resize(count);
	shs.resize(count);
	scales.resize(count);
	...
	filter_3Ds.resize(count);
	temp_scales.resize(count);
	...
	for (int k = 0; k < count; k++)
	{
		...
		// filter_3D
		filter_3Ds[k] = points[i].filter_3D;
		
		// Exponentiate scale
		for(int j = 0; j < 3; j++)
			{scales[k].scale[j] = exp(points[i].scale.scale[j]);
			// Mip splatting
			scales[k].scale[j] = scales[k].scale[j] * scales[k].scale[j] + filter_3Ds[k] * filter_3Ds[k];
			scales[k].scale[j] = sqrtf(scales[k].scale[j]);
			}
		...
		for (int j=0; j < 3; j++)
			{temp_scales[k].scale[j] = exp(points[i].scale.scale[j]);
			temp_scales[k].scale[j] = temp_scales[k].scale[j] * temp_scales[k].scale[j];
			}
		float det1 = temp_scales[k].scale[0] * temp_scales[k].scale[1] * temp_scales[k].scale[2];
		for (int j=0; j < 3; j++)
			temp_scales[k].scale[j] = temp_scales[k].scale[j] + filter_3Ds[k] * filter_3Ds[k];

		float det2 = temp_scales[k].scale[0] * temp_scales[k].scale[1] * temp_scales[k].scale[2];
		float coef = sqrtf(det1 / det2);
		opacities[k] = coef * opacities[k];
...
sibr::GaussianView::GaussianView(const sibr::BasicIBRScene::Ptr & ibrScene, uint render_w, uint render_h, const char* file, bool* messageRead, int sh_degree, bool white_bg, float kernel_size, bool useInterop, int device) :
	_scene(ibrScene),
	...
	sibr::ViewBase(render_w, render_h)
{
	...
	std::vector<SHs<3>> shs;
	// std::vector<float> subpixel_offset;
	if (sh_degree == 1)
	{
	...
	// CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&subpixel_offset_cuda, sizeof(float) * 2 * P));
	// CUDA_SAFE_CALL_ALWAYS(cudaMemset(subpixel_offset_cuda, 0, sizeof(float) *2* P));
	// CUDA_SAFE_CALL_ALWAYS(cudaMemcpy(subpixel_offset_cuda, subpixel_offset.data(), sizeof(float) * P, cudaMemcpyHostToDevice));
...
void sibr::GaussianView::onRenderIBR(sibr::IRenderTarget & dst, const sibr::Camera & eye)
{
	...
	else
	{
		...
		CUDA_SAFE_CALL_ALWAYS(cudaMalloc((void**)&subpixel_offset_cuda, sizeof(float) * 2 * _resolution.x() * _resolution.y()));
		CUDA_SAFE_CALL_ALWAYS(cudaMemset(subpixel_offset_cuda, 0, sizeof(float) * 2 * _resolution.x() * _resolution.y()));
		...
		CUDA_SAFE_CALL_ALWAYS(cudaFree(subpixel_offset_cuda));
	}
