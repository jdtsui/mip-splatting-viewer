...
namespace sibr {
	...
	void InteractiveCameraHandler::fromCamera(const sibr::InputCamera & cam, bool interpolate, bool updateResolution) {
		...
		// idealCam.setFocal(_currentCamera.focal()); // NOTE: do not change fov
	...
	void InteractiveCameraHandler::snapToCamera(const int i) {
		if (!_interpPath.empty()) {
			...
			_currentCamId = nearestCam; // NOTE: update the camera id
			_interpPath[nearestCam]->fovy(_currentCamera.fovy()); // NOTE: do not change the fov
			fromCamera(*_interpPath[nearestCam], true, true); // NOTE: update the camera resolution 
		}
	}
	...
	void InteractiveCameraHandler::update(const sibr::Input & input, float deltaTime, const sibr::Viewport & viewport) {
		...
		else if (input.key().isActivated(sibr::Key::Equal)) {
			_currentCamId = sibr::clamp(_currentCamId + 1, 0, int(_interpPath.size()) - 1);
			snapToCamera(_currentCamId);
		}
		else if (input.key().isActivated(sibr::Key::Minus)) {
			_currentCamId = sibr::clamp(_currentCamId - 1, 0, int(_interpPath.size()) - 1);
			snapToCamera(_currentCamId);
		}
